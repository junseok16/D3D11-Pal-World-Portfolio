...
{
#ifdef _DEBUG
	if (m_eCameraType == ECameraType::DEBUG_) { return 0; }
	...
	m_tCameraCullSphere.Transform(m_tWorldCameraCullSphere, matScaleKilled);
	m_tCameraCullFrustum.Transform(m_tWorldCameraCullFrustum, matScaleKilled);
#endif // _DEBUG
...
{
	if (m_eCameraType == ECameraType::DEBUG_) { return; }
	...
	DX::Draw(m_spPrimitiveBatch.get(), m_tWorldCameraCullSphere, m_bCollided ? Colors::Red : Colors::Black);

	DX::Draw(m_spPrimitiveBatch.get(), m_tWorldCameraCullFrustum, m_bCollided ? Colors::Red : Colors::Black);
...
void Engine::CCameraComponent::RecalcCameraMatrix()
{
	...
	case Engine::ECameraType::DEBUG_:
	{
		// 뷰 행렬을 구합니다.
